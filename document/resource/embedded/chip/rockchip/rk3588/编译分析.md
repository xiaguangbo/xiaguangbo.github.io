...
recovery.img
    ...
    device/rockchip/common/mk-fitimage.sh 106：$TOP_DIR/rkbin/tools/mkimage -f $target_its_file  -E -p 0x800 $OUTPUT_TARGET_IMAGE
    能启动，但是会报
...
```
最少需要 MiniLoaderAll.bin、uboot.img、boot.img、rootfs.img。去掉 userdata	RESERVED 后会出现一些功能加载失败的情况，不影响
