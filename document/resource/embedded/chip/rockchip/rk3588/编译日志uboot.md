...
++ mktemp
+ local tmp_file=/tmp/tmp.DPEvIRsIcG
+ uniq
...
+ '[' '!' -f ']'
+ read line
++ echo Documentation Kbuild Kconfig Licenses MAINTAINERS Makefile PREUPLOAD.cfg README System.map api arch bl31.elf bl31_0x00040000.bin bl31_0x000f0000.bin bl31_0xff100000.bin board cmd common config.mk configs disk doc drivers dts env examples fit fs include lib make.sh net post scripts snapshot.commit spl tee.bin test tools tpl u-boot u-boot-dtb.bin u-boot-dtb.img u-boot-nodtb.bin u-boot.bin u-boot.cfg u-boot.cfg.configs u-boot.dtb u-boot.img u-boot.itb u-boot.its u-boot.lds u-boot.map u-boot.srec u-boot.sym Copyright '(C)' 2020 Rockchip Electronic Co.,Ltd
++ sed -n /incbin/p
++ awk -F '"' '{ printf $2 }'
++ tr -d ' '
+ file=
+ '[' '!' -f ']'
+ read line
++ echo Documentation Kbuild Kconfig Licenses MAINTAINERS Makefile PREUPLOAD.cfg README System.map api arch bl31.elf bl31_0x00040000.bin bl31_0x000f0000.bin bl31_0xff100000.bin board cmd common config.mk configs disk doc drivers dts env examples fit fs include lib make.sh net post scripts snapshot.commit spl tee.bin test tools tpl u-boot u-boot-dtb.bin u-boot-dtb.img u-boot-nodtb.bin u-boot.bin u-boot.cfg u-boot.cfg.configs u-boot.dtb u-boot.img u-boot.itb u-boot.its u-boot.lds u-boot.map u-boot.srec u-boot.sym
++ sed -n /incbin/p
++ tr -d ' '
++ awk -F '"' '{ printf $2 }'
+ file=
+ '[' '!' -f ']'
+ read line
++ echo Documentation Kbuild Kconfig Licenses MAINTAINERS Makefile PREUPLOAD.cfg README System.map api arch bl31.elf bl31_0x00040000.bin bl31_0x000f0000.bin bl31_0xff100000.bin board cmd common config.mk configs disk doc drivers dts env examples fit fs include lib make.sh net post scripts snapshot.commit spl tee.bin test tools tpl u-boot u-boot-dtb.bin u-boot-dtb.img u-boot-nodtb.bin u-boot.bin u-boot.cfg u-boot.cfg.configs u-boot.dtb u-boot.img u-boot.itb u-boot.its u-boot.lds u-boot.map u-boot.srec u-boot.sym Simple U-boot fit source file containing ATF/OP-TEE/U-Boot/dtb/MCU
++ sed -n /incbin/p
++ tr -d ' '
++ awk -F '"' '{ printf $2 }'
+ file=
+ '[' '!' -f ']'
+ read line
++ echo Documentation/ Licenses/ api/ arch/ board/ cmd/ common/ configs/ disk/ doc/ drivers/ dts/ env/ examples/ fit/ fs/ include/ lib/ net/ post/ scripts/ spl/ test/ tools/ tpl/
++ sed -n /incbin/p
++ tr -d ' '
++ awk -F '"' '{ printf $2 }'
+ file=
...
Created:         Thu Dec  1 19:09:10 2022
 Image 0 (uboot)
  Description:  U-Boot
  Created:      Thu Dec  1 19:09:10 2022
  ...
  Hash algo:    sha256
  Hash value:   b08efe298fe421392c195b03f1c3f29b38c42a3afad92ee750f940f9d06f3015
 Image 1 (atf-1)
  Description:  ARM Trusted Firmware
  Created:      Thu Dec  1 19:09:10 2022
  Type:         Firmware
...
+ return
+ fit_msg_loader
+ ls rk3588_spl_loader_v1.08.111.bin
++ ls rk3588_spl_loader_v1.08.111.bin
+ LOADER=rk3588_spl_loader_v1.08.111.bin
+ ls '*idblock*.img'
+ grep -q '^CONFIG_FIT_SIGNATURE=y' .config
+ echo 'Image(no-signed): rk3588_spl_loader_v1.08.111.bin (with spl, ddr...) is ready'
